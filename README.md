...
| **ZRAM Manager** | v3 ([3 releases](https://github.com/abr60/my-ksu-module-kit/releases)) | Rei Ryuki (The Fixer) / https://github.com/reiryuki/ZRAM-Swap-Configurator-Magisk-Module (MIT) — WebUI fork by abr60 | 
...
⚠️ **Gotcha:** The original v3.7 module defaults to **100% of RAM** if `optionals.prop` is unset — this can leave little room on a 4 GB device. The ZRAM Manager fork defaults to **50%** and adds an interactive WebUI.

📦 **ZRAM Manager (WebUI Fork)**
A WebUI-based fork of reiryuki's ZRAM-Swap-Configurator. Opens from the KernelSU manager to let you adjust zram size, algorithm, priority, swappiness, swap_ratio, min/extra free kbytes and LMKD props — with **Apply & Reboot** (saves config and reboots). Default 50 % on first install; pre‑seed `/sdcard/optionals.prop` with `zram.resize=65%` + `zram.algo=lz4` to preserve your existing tuning.
